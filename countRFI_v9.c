...
// 2014.04.01: also output the rms to calculate the energy
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>

int cal_devi(float *data, int num, float *ave, float *devi, float *rms)
// using an iterative definition of the standard deviation; calculating the mean and standard deviation
{
	...
	{
		mean += data[i];
		temp_rms += data[i]*data[i];
	}
	mean = mean/num;
	temp_rms = sqrt(temp_rms/num);
...
int iter_devi(float *data, int num, float *ave, float *devi, float *rms)
{
	int i;
	
	// initialize
	float mean,sigma,temp_rms;
	cal_devi(data, num, &mean, &sigma, &temp_rms);
	...
	while (count1 > count2)
	{
		...
		cal_devi(temp, j, &mean, &sigma, &temp_rms);
...
int main (int argc, char *argv[])
{
	...
	float ave, devi, rms;
	iter_devi(data, num, &ave, &devi, &rms);
